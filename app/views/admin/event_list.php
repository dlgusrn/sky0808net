...
// 교회 선택 조건문
$church_select = isset ( $_GET['church'] ) ? $_GET['church'] : '';
if ( ! empty ( $church_select ) ){
    $where = "WHERE church = '$church_select' " ;
} else {
    $where = "WHERE church IN ( 'sky', 'beer' )" ;
}

// 게시물 번호를 위한 DB 조회
$num = $DB->query( "SELECT * FROM live_list " . $where )->rowCount() ;
...
$listSql = $DB->query( "SELECT * FROM live_list " . $where . " ORDER BY idx DESC LIMIT " . $start_num . ', ' . $page_set ) ;
...
<main class="live-main">
    <div class="main-row">
        ...
        <div class="nav-right">
            <h4 class="main-title">행사 영상 관리</h4>
            <div class="admin-row">
                <table class="table table-hover text-center">
                        ...
                        <tr>
                            ...
                            <th width="9%">
                                <select class="form-select form-select-sm" id="church_select">
                                    ...
                                    <option value="sky" <?='sky' == $church_select ? 'selected' : ''?>>하늘문</option>
                                    <option value="beer" <?='beer' == $church_select ? 'selected' : ''?>>브엘성회</option>
                                </select>
                            </th>
                            <th>제목</th>
                            <th width="16%">수정일자</th>
                    ...
                    <? else : ?>
                        ...
                        <tr>
                            ...
                            <td><?=$church?></td>
                            <td><a class="link-dark" href="./live_read.html?idx=<?=$row['idx']?>&church=<?=$row['church']?>"><?=$date[0]?>년 <?=$date[1]?>월 <?=$date[2]?>일(<?=$yoil[date('w', strtotime($row['title']))]?>) <?=$row['worship']?></a></td>
